...
在 Unity 菜单栏中选择 Window -> Package Manager, 选择添加方式为 `Add package from git URL` , 在地址栏中输入 `https://github.com/xindong/TAPSDK_UPM.git#1.2.1` 和 `https://github.com/xindong/XDSDK_UPM.git#3.6.0` 即可。

> 注意：不同 Unity 版本 UI 显示可能略有差别

#### 1.3 通过本地源码导入

在 ProjectPath 目录下新建 XDSDK、TapSDK 两个文件夹，拷贝 [XDSDK源码](https://github.com/xindong/XDSDK_UPM.git#3.6.0) 、[TapSDK源码](https://github.com/xindong/TAPSDK_UPM.git#1.2.1) 到指定目录中。
